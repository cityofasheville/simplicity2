...
const AddressesByStreet = (props) => {
  ...
  const mapData = props.data.addresses_by_street.map(item => (Object.assign({}, item, { popup: `<b>Address</b><div>${item.address}</div><div>${item.city}, NC ${item.zipcode}</div><br /><b>Owner</b><div>${item.owner_name}</div><div>${item.owner_address}</div><div>${item.owner_cityname}, ${item.owner_state} ${item.owner_zipcode}</div>` })
  ));
