		...
		{({ loading, error, data }) => {
			...
			const dataForTable =
				props.location.query.mode === "expenditures" || props.location.query.mode === undefined
					? data.budgetTrees.expenseTree.children
					: data.budgetTrees.revenueTree.children;
			...
			for (let i = 0; i < budgetYears.length + 1; i += 1) {
				last4YrBudgetTypes.push(
					i < budgetYears.length - 1
						? content.actual
			...
			const getChangeHeader = () => (
				<div>
					{content.change_from} <br /> {content.past_year}
				</div>
			);
