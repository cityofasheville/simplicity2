...
const calculateBounds = (points) => {
  ...
  if (points.length > 0) {
    ...
    return [[points[yMinIndex].y, points[xMinIndex].x], [points[yMaxIndex].y, points[xMaxIndex].x]];
  }
  return null;
};
    ...
    props.latitude.map((y, index) =>
      Object.assign(
        ...
        {},
        {
          ...
          popup: `<div><b>${props.display_name}</b><p>Latitude: ${y}</p><p>Longitude: ${
            props.longitude[index]
          }</p></div>`,
        }
      ));
            ...
            {props.project_webpage_more_information !== null && (
              <div className="row">
                <div className="col-sm-12" style={{ marginTop: '5px' }}>
                  <div className="pull-left" style={{ marginRight: '10px', marginBottom: '20px' }}>
                    {' '}
                    <a
                      ...
                      target="_blank"
                    >
                      <Icon path={IM_SPHERE3} size={20} /> Project Website
                    </a>
                  </div>
                </div>
              </div>
            ...
            <div className="capital-project__specs row">
              <div className="col-xs-5">
                <div className="" style={{ marginBottom: '10px' }}>
                  <div style={{ color: '#676873' }}>Current Project Budget</div>
                  <div>
                ...
                {props.show_pm_fields && (
                  <div className="" style={{ marginBottom: '10px' }}>
                    <div style={{ color: '#676873' }}>Spent</div>
                    <div>
                      <strong>
                        {['$', parseInt(props.total_spent).toLocaleString()].join('')}
                      </strong>
                    </div>
                  </div>
                )}
                {props.show_pm_fields && (
                  <div className="" style={{ marginBottom: '10px' }}>
                    <div style={{ color: '#676873' }}>Under contract</div>
                    <div>
              ...
              <div className="col-xs-7">
                <div className="" style={{ marginBottom: '10px' }}>
                  <div style={{ color: '#676873' }}>Zip code</div>
                  <div>
                ...
                {props.show_pm_fields && (
                  <div className="" style={{ marginBottom: '20px' }}>
                    <div style={{ color: '#676873' }}>Estimated Construction Timeframe</div>
                    <div>
                      <strong>
                        {props.status !== null && props.status.indexOf('Completed') > -1
                          ? ['Completed', props.actual_construction_end].join(' ')
                          : props.target_construction_start === null
                            ? props.target_construction_end
                            : [props.target_construction_start, props.target_construction_end].join(' - ')}
                      </strong>
                    </div>
                  </div>
                )}
            ...
            {props.show_pm_fields &&
              props.status !== null && (
                <div className="capital-project__status">
                  <div className="header">Project phase</div>
                  <div className="project-status">
                    <Icon
                      path={IM_CIRCLE2}
                      size={25}
                      color={getStageNumber(props.status) >= 1 ? phaseColor(1) : '#ecf0f1'}
                    />
                  </div>
                  <div className="project-status">
                    <Icon
                      path={IM_CIRCLE2}
                      size={25}
                      color={getStageNumber(props.status) >= 2 ? phaseColor(2) : '#ecf0f1'}
                    />
                  </div>
                  <div className="project-status">
                    <Icon
                  ...
                  {props.status !== 'Ongoing' && (
                    <div className="project-status">
                      <Icon
                        path={IM_CIRCLE2}
                        size={25}
                        color={getStageNumber(props.status) >= 4 ? phaseColor(4) : '#ecf0f1'}
                      />
                    </div>
                  )}
                  <div
