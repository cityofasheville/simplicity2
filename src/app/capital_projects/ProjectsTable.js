  ...
  dataColumns = state => [
    {
      ...
      Cell: row => (
        <span>
          <span title={mapProjectToCategory(row.original)}>
            {getIcon(row.original.category, row.isExpanded)}
          </span>
          {row.original.category.indexOf('Bond') > -1 && (
            <span title="Bond project" style={{ marginLeft: '3px' }}>
              <Icon
                path={LI_BOLD}
                size={16}
                color={row.isExpanded ? '#fff' : '#4077a5'}
                viewBox="0 0 24 24"
              />
            </span>
          )}
