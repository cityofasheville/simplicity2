...
function ProjectsTable(props) {
	...
	const uniqueStatuses = ["All", "Proposed", "Planning", "Design", "Construction", "Completed"];
	const [pageNum, setPageNum] = useState(1);
	const [sorting, setSorting] = useState([]);
	const rowOptions = [10, 25, 50, 100];
	const [columnFilters, setColumnFilters] = useState(getInitialFiltersFromURL());
	...
	useEffect(() => {
		...
		const statusFilterIndex = initialFilters.findIndex((f) => f.id === "status");
		...
		const filtered = initialFilters.filter((obj) => !otherFilters.includes(obj.id));
	...
	useEffect(() => {
		...
		const allFilterIDs = ["status", "name", "zip_code", "total_project_funding_budget_document", "encumbered", "spent"];
	...
	const table = useReactTable({
		...
		onSortingChange: (updater) => {
			const resolvedSorting = typeof updater === "function" ? updater(table.getState().sorting) : updater;
			if (resolvedSorting.length > 0) {
				const { id, desc } = resolvedSorting[0];

				const sortDirection = desc ? "desc" : "asc";

				updateURL(location.search, [{ id: "sort_column", value: id }], ["sort_column"]);
				updateURL(location.search, [{ id: "sort_order", value: sortDirection }], ["sort_order"]);
			} else {
				// Sorting is cleared (default)
				updateURL(location.search, [{ id: "sort_column", value: "" }], ["sort_column"]);
				updateURL(location.search, [{ id: "sort_order", value: "default" }], ["sort_order"]);
			}
		},
							...
							<table id="CIPDataTable" className="table-fixed w-full shadow-md">
															...
															<>
																...
																<div className="p-1">
																	{header.column.getCanFilter() ? (
																		...
																		<div className="w-full h-8 box-border m-1"></div>
																	)}
																</div>
															</>
														)}
								...
								<tbody>
									{table.getRowModel().rows.map((row) => {
										return (
											<tr className="even:bg-white odd:bg-gray-50" key={row.id}>
												{row.getVisibleCells().map((cell) => {
													return (
														<td className=" py-1 px-2 border" key={cell.id}>
...
function Filter({ column }) {
	...
	return (
		<div>
			<DebouncedInput
				className="w-full box-border border rounded p-1"
				onChange={(value) => column.setFilterValue(value)}
				placeholder={`Search...`}
				type="text"
				value={columnFilterValue || ""}
				label={`Search ${column.id}`}
			/>
		</div>
	);
}

function DebouncedInput({ value: initialValue, onChange, debounce = 500, label, ...props }) {
	...
	return <input {...props} aria-label={label} value={value} onChange={(e) => setValue(e.target.value)} />;
}
