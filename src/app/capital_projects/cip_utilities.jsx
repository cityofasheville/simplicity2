...
export const getCategoryBarChartData = (projectData, categories, mode) => {
	...
	for (let project of projectData) {
		if (categories.includes(project.category)) {
			if (mode !== "bond" || (mode === "bond" && project.type.toLowerCase() === "bond")) {
	...
	for (let project of projectData) {
		if (categories.includes(project.category)) {
			if (mode !== "bond" || (mode === "bond" && project.type.toLowerCase() === "bond")) {
				...
				if (
					project.total_project_funding_budget_document !== null &&
					project.total_project_funding_budget_document.trim() !== ""
				) {
					let cleanBudgetAmount = project.total_project_funding_budget_document.replace(/ /g, "");
					let allocated =
						cleanBudgetAmount.indexOf("$") === 0
							? cleanBudgetAmount.slice(1).split(",").join("")
							: cleanBudgetAmount.split(",").join("");
					if (!isNaN(allocated)) {
...
export const urlCategory = (category) => encodeURIComponent(category);
