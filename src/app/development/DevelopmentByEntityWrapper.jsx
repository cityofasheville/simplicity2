...
function DevelopmentByEntityWrapper(props) {
	...
	const [timeSpan, setTimeSpan] = useState([NEARBY_START_DATE, TODAY]);
	...
	function getFormattedExtent() {
		if (props.location.query.entity === "address" && radius === "0") {
			return [moment.utc(AT_START_DATE).format("YYYY-MM-DD"), moment.utc(TODAY).format("YYYY-MM-DD")];
		} else {
			return [moment.utc(timeSpan[0]).format("YYYY-MM-DD"), moment.utc(timeSpan[1]).format("YYYY-MM-DD")];
		}
	}
		...
		<div>
			...
			<div className="row">
				<div className="col-sm-8">
					<p>
						The map, list, and chart represent all development permit types, of which there are over 40. Some permit
						types included are:
					</p>
					<ul>
						<li>Residential</li>
