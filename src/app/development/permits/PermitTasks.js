...
const getDate = d => moment(new Date(d)).format('MMM DD, YYYY');

const PermitTasks = props => (<Query
...
>
  {({ loading, error, data }) => {
    ...
    const steps = data.permit_tasks.slice();
    ...
    steps.sort((a, b) =>
      new Date(b.current_status_date).getTime() - new Date(a.current_status_date).getTime());
