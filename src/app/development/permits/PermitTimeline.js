...
class PermitTimeline extends React.Component {
	...
	renderContent() {
		...
		if (this.props.currentStatusItem !== undefined && !this.props.currentStatusItem.closed) {
			datesToUse = this.props.formattedPermit.orderedDates.concat([
				{
					accelaLabel: "dummy",
					dateInput: null,
					displayLabel: "",
		...
		const getX = (dateObj, i) => midpointX + ((i % datesToUse.length) - midRowIndex) * (padding + eachWidth);
		const svgHeight = 120;

		let titleText = datesToUse
			.filter((d) => d.accelaLabel !== "dummy")
			.map((d) => {
				const formattedDate = this.props.dateFormatter(d.dateInput);
				return `${formattedDate} ${d.displayLabel}`;
			})
			.join(" , ");
		...
		return (
			<div>
				<span id="timelineInfo" className="sr-only">
					{titleText}
				</span>

				<svg aria-labelledby="timelineInfo" title={titleText} role="img" height={svgHeight} width={timelineWidth}>
					{datesToUse.map((d, i, datesArray) => {
						...
						return (
							<g key={d.accelaLabel}>
								{d.accelaLabel !== "dummy" && (
									<circle cx={thisX} cy={circleY} r={pointRadius} fill={this.props.formattedPermit.trcType.color} />
								)}
								{d.accelaLabel === "dummy" && (
									<path
										d={`M${thisX},${circleY - pointRadius / 2} L${thisX},${circleY + pointRadius / 2} L${
											thisX + pointRadius
										},${circleY} z`}
