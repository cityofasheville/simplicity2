...
class PermitsIndex extends React.Component {
	constructor(props) {
		...
		if (currentUrlParams.has("rangeFrom") && currentUrlParams.has("rangeThrough")) {
			...
			defaultExtent = [timeMonth.offset(timeDay.floor(new Date()), -12).getTime(), timeDay.floor(new Date()).getTime()];
		} else {
			defaultExtent = [this.props.initialBrushExtent[0], this.props.initialBrushExtent[1]];
		}
	...
	onDateRangeChange(newExtent) {
		...
		if (history.pushState) {
			let newurl =
				window.location.protocol + "//" + window.location.host + window.location.pathname + `?${currentUrlParams}`;
			window.history.pushState({ path: newurl }, "", newurl);
		}
	...
	render() {
		return (
			<div className="">
				<h1 className="text-coa-blue-medium text-3xl mb-3">All Permit Applications</h1>
				<hr />
				<h2 className="mt-8">Filter Permits by Date Applied</h2>
				<p>
					If you require access to the full permits dataset (i.e. more than is available with a single query on this
					page), please visit the City of Asheville's{" "}
					<a
...
PermitsIndex.defaultProps = {
	initialBrushExtent: [timeMonth.offset(timeDay.floor(new Date()), -1).getTime(), timeDay.floor(new Date()).getTime()],
