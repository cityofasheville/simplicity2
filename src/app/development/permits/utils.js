...
export const permitFieldFormats = [
		...
		formatFunc: (d) =>
			d === "Yes" ? (
				<span className="permit-icon-boolean">
					<Icon path={IM_WATER} viewBox="0 0 576 512" />
					<span className="ml-2">Aquatic buffer located on this property</span>
				</span>
			) : null,
	},
	{
		...
		formatFunc: (d) => {
			...
			return (
				<span className="permit-icon-boolean">
					<Icon path={IM_TINT} viewBox="0 0 352 512" />
					{text}
				</span>
			);
		},
	...
	},
	{
		...
		formatFunc: (d) =>
			d === "Yes" ? (
				<span className="permit-icon-boolean">
					<Icon path={IM_RECYCLE} viewBox="0 0 512 512" />
	...
	},
	{
		...
		formatFunc: (d, permit) =>
			permit.civic_address_id ? <a href={`/address?id=${permit.civic_address_id}`}>{d}</a> : d,
	},
	{
		...
		formatFunc: (d, permit) => {
			...
			return (
				<div>
					<TypePuck typeObject={permit.trcType} size={30} hover={false} />
					<span style={{ padding: "0 0 0 1rem" }}>
						<a href="/development/major#types" target="_blank" rel="noopener noreferrer">
							{permit.trcType.id}
						</a>
					</span>
				</div>
	...
	},
	{
		...
		formatFunc: (d, permit) => {
			...
			return (
				<a href={`mailto:${emailLinkText}?subject=Inquiry from SimpliCity user about ${permit.permit_number}`}>
					{emailLinkText}
				</a>
			);
		},
	},
	{
		...
		formatFunc: (d) =>
			d
				...
				.filter((z) => z.length > 0)
				.map((zone, i) => (
					<a href={getZoningLink(zone) || undefined} target="_blank" rel="noopener noreferrer" key={zone}>
						{`${i > 0 ? ", " : ""}${zone}`}
					</a>
				)),
	},
	...
	},
	{
		...
		formatFunc: (d) =>
			d === "Yes" ? (
				<span className="permit-icon-boolean">
					<Icon path={IM_LANDMARK} viewBox="0 0 512 512" />
					Historic district
				</span>
			) : null,
	},
	{
		...
		formatFunc: (d) =>
			d === "Yes" ? (
				<span className="permit-icon-boolean">
					<Icon path={IM_FISH} viewBox="0 0 576 512" />
					River district
				</span>
			) : null,
	},
	{
		...
		formatFunc: (d) =>
			d === "Yes" ? (
				<span className="permit-icon-boolean">
					<Icon path={IM_MONUMENT} viewBox="0 0 384 512" />
					Historic landmark
				</span>
			) : null,
	},
