...
class FlowDiagram extends React.Component {
	constructor() {
		...
		this.numLevels = yVals.filter((value, index, nodeArray) => nodeArray.indexOf(value) === index).length;
					...
					<g>
						{nodes.map((d) =>
							dimensions.width > 767 ? (
								...
								<SmallNode node={d} yOffset={yOffset} edgeStroke={edgeStroke} key={d.id} clickAction={this.showModal} />
							)
						)}
					</g>
				</svg>
