...
export const getTRCTypeFromPermit = (permit) => {
  if (permit.permit_group === 'Planning') {
    return Object.values(trcProjectTypes).find(
      (type) =>
        type.permit_type === permit.permit_type && type.permit_subtype === permit.permit_subtype,
    );
  }
  return null;
};
...
export const getDagreGraph = (nodes, links, nodeSize) => {
  ...
  */
  ...
  nodes.forEach((node) => {
    g.setNode(node.id, Object.assign({ width: nodeSize, height: nodeSize }, node));
  });

  links.forEach((link) => {
    g.setEdge(link.source, link.target, link);
  });
...
export function getNodes(dagreGraph, visWidth, nodeHeight, nodePadding) {
  ...
  */
  ...
  const returnNodes = Object.values(dagreGraph._nodes).map((d) => {
    ...
    returnNode.coincidents = []
      .concat(Object.values(dagreGraph._nodes))
    ...
    returnNode.wrap =
      (visWidth - (nodePadding + nodePadding * returnNode.numPerRow)) / returnNode.numPerRow;
...
export function getLinks(inputLinks, nodes, edgePadding, edgeStroke) {
  const linkValues = JSON.parse(JSON.stringify(inputLinks)).map((link) => {
  ...
  withoutParallels.forEach((link) => {
    ...
    link.x1Coincidents = withoutParallels.filter(
      (otherLink) => otherLink.x1 === link.x1 && otherLink.y1 === link.y1,
    );
    link.x1CoincidentIndex = link.x1Coincidents.findIndex(
      (coincident) =>
    ...
    link.x2Coincidents = withoutParallels.filter(
      (otherLink) => otherLink.x2 === link.x2 && otherLink.y2 === link.y2,
    );
    link.x2CoincidentIndex = link.x2Coincidents.findIndex(
      (coincident) =>
        coincident.source === link.source &&
        coincident.target === link.target &&
        coincident.id === link.id,
  ...
  return withoutParallels.map((link) => {
    ...
    rObj.x1 =
      link.x1 +
      link.x1CoincidentIndex * paddingAndStroke -
      ((link.x1Coincidents.length - 1) * paddingAndStroke) / 2;
    rObj.x2 =
      link.x2 +
      link.x2CoincidentIndex * paddingAndStroke -
      ((link.x2Coincidents.length - 1) * paddingAndStroke) / 2;
    return rObj;
  });
}
