...
function TimeSlider({
	defaultBrushExtent = [timeWeek.offset(timeDay.floor(new Date()), -1).getTime(), timeDay.floor(new Date()).getTime()],
	...
	const [xSpan, setXSpan] = useState([timeYear.offset(spanEnd, -1 * xSpanYears).getTime(), spanEnd]);
		...
		(proposedExtent, snap = false) => {
			...
			if (proposedExtent) {
				...
				if (snap) {
					...
					newExtent = [timeFunc.floor(proposedExtent[0]).getTime(), timeFunc.floor(proposedExtent[1]).getTime()];
				}
				...
				if ((+selectedRange > +maxDaysAllowedToQuery || +newExtent[0] >= +newExtent[1]) && selectedRange !== 0) {
			...
			<ErrorBoundary>
				<div>
					<div
						...
						className="text-sm ml-0 pl-2 mr-2 mb-4"
						style={{
					...
					>
						<fieldset>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-2">
								<div className="min-w-0 flex flex-col gap-1 md:flex-row md:items-center md:gap-0">
									...
									<input
										...
										name="startdate"
										className="input lg:input-sm grow min-w-0"
										size={10}
						...
						axes={[
							{
								...
								tickFormat: (d) => {
									return (
										<text textAnchor="middle" style={{ fontSize: "0.70em", left: "-14px" }} transform="rotate(-45)">
