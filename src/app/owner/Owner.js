  ...
  return (
    <div>
      <PageHeader
        ...
        h3="About this owner's properties"
        dataType="Owner"
        icon={<Icon path={IM_USER} size={50} />}
      >
      ...
      <div className="row">
        <div className="col-xs-12">
          <div className="pull-left" style={{ marginTop: '10px', marginBottom: '15px' }}>
            <EmailDownload
              downloadData={props.data.properties}
              fileName="properties_by_owner.csv"
              text="Download CSV"
            />
          </div>
          ...
          {props.location.query.view === 'map' ? (
            ...
            <div style={{ marginTop: '10px' }}>
              <ExpandableAccessibleReactTable
                ...
                defaultPageSize={
                  props.data.properties.length <= 20 ? props.data.properties.length : 20
                }
                filterable
...
const OwnerWithData = graphql(propertyQuery, {
  options: ownProps => ({
    variables: {
      pins:
        ownProps.location === undefined
          ? ownProps.pins
          : ownProps.location.query.id.trim().split(','),
    },
  }),
})(Owner);
