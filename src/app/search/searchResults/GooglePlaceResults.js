	...
	const renderResults = () => {
		...
		return (
			<div>
				...
				<p>
					<i>{props.location.query.placeSearch}</i>, the address from Google associated with the search{" "}
					<i>{props.location.query.search}</i>, returned multiple results in SimpliCity. Explore the potential matches
					below.
				</p>
...
export default graphql(searchQuery, {
	...
	options: (ownProps) => ({
		variables: { searchString: ownProps.location.query.placeSearch.trim(), searchContexts: getEntitiesToSearch() },
	}),
})(GooglePlaceResults);
