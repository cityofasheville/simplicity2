...
const DetailsGrouping = (props) => {
  ...
  for (let i = 0, j = props.dataValues.length; i < j; i += subArraySize) {
        ...
        <div>
          {props.hasTitle && (
            <div className={styles.titleDiv}>
              {props.hasTitleIcon && <i className={['fa fa-', props.titleIcon].join('')} />}{' '}
              {props.title}
            </div>
          )}
        </div>
        ...
        <div key={i} className={['col-xs-', props.colWidth].join('')}>
          {values.map((value, j) => (
            <DetailsFormGroup
              ...
              hasLabel={hasLabels}
              hasIcon={hasIcons}
              label={hasLabels ? subLabelsArrays[i][j] : ''}
              icon={hasIcons ? subIconsArrays[i][j] : ''}
              value={value}
            />
          ))}
        </div>
