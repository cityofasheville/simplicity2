    ...
    return (
      <div role="img" aria-label={this.props.altText} tabIndex={0}>
        <NetworkFrame
          ...
          hoverAnnotation
          margin={{
            top: -20, right: this.props.width / 9, bottom: 25, left: this.props.width / 9,
          }}
          networkType="sankey"
          nodeIdAccessor="id"
          nodes={graph.nodes}
          ...
          svgAnnotationRules={(d) => {
            ...
            if (this.state.hover === d.d.name) {
              ...
              return (
                <text key={key} style={style} x={d.d.x - 4 * label.length} y={d.d.y} opacity={0.5}>
                  &#8594; {label}
                </text>
              );
          ...
          edgeStyle={(d) => {
            ...
            const opacity =
              this.state.hover === d.source.name || this.state.hover === d.target.name
                ? '0.75'
                : '0.15';
