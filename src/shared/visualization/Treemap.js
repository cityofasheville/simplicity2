...
const getTopLevelOnly = data => data.map(item => Object.assign({}, item, { children: null }));
  ...
  return (
    <text x={x + 4} y={y + 17} fill={fill} stroke="none" style={{ fontWeight: 'bold' }}>
      {lines.map((word, index) => (
        <tspan key={[name, 'line', index].join('_')} dy={16} x={x + 4}>
          {word}
        </tspan>
      ))}
    </text>
    ...
    return (
      <g>
        ...
        <path
          ...
          style={{
            cursor: 'pointer',
            fill: differenceColors
              ? getFill(delta)
              : COLORS[Math.floor(index % root.children.length)],
            stroke: '#000',
          }}
        />
