...
export const labelOrder = (formattedData, valueAccessor = 'value') =>
  JSON.parse(JSON.stringify(formattedData))
    ...
    .map((item) => {
      item.sum = formattedData.filter(d => d.label === item.label).reduce((total, num) => {
        const returnObj = {};
